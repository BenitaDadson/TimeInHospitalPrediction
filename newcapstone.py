...
with st.sidebar:
    ...
    agenumeric = st.number_input("Age", min_value=0, max_value=100, value=0)
    ...
    number_diagnoses = st.slider("Number of Diagnoses", min_value=0, max_value=15, value=0)
    ...
    time_in_hospital = st.slider("Time in Hospital", min_value=0, max_value=14, value=0)
    hospital_service = st.sidebar.slider('Hospital Service', 0, 7, key='hospital_service_input')
    discharge_status = st.selectbox("Discharge Status",
                                    ["None", "Discharged to home", "Discharged/transferred", "Hospice / home",
                                     "Hospice / medical facility", "Left AMA", "Still patient/Expected Return"])
...
inputs = {
    ...
    'diabetesMedYes': 1 if diabetes_med == "Yes" else 0,
